    ...
    return (
        <Stack sx={{backgroundColor: '#111823', width: '15rem', height: '100%'}} justifyContent={'space-between'}>
            <Box>
                <Stack direction={'row'} alignItems={'center'} justifyContent={'space-evenly'} padding={'2rem'}>
                    <Image
                        ...
                        alt="logo"
                    />
                    <Typography color={'white'} variant="h5" fontWeight={'bold'}>WIZE<span>PROMPT</span></Typography>
                </Stack>
                <Stack gap={1} alignItems={'center'}>
                    {sidebarItems.map(({ name, href, icon: Icon}) => (
                            <Button href={href} key={name} variant="outlined" sx={{
                                width: '13rem', 
                                textTransform: 'none',
            ...
            <Box display={'flex'} alignItems={'center'} justifyContent={'center'} padding={'2rem'}>
                <Button variant="contained" href="/welcome" sx={{
                    bgcolor: '#E93D44',
                    '&:hover': {bgcolor: 'red'}
